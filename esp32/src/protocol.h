// Wire protocol for the panel's command channel - mirrors protocol.py in
// the repo root byte-for-byte. Kept transport-agnostic here too: this file
// only deals with parsing bytes already in memory, never touches
// BluetoothSerial/BLE directly, so the same code serves both firmware
// variants (see main_classic.cpp/main_ble.cpp for how each one gets bytes
// into a buffer in the first place).
//
...
// width*height RGB triplets - mirrors parse_ppm() in protocol.py exactly,
// including skipping a '#'-prefixed comment line (some bundled assets were
// exported from GIMP, which adds one) like whitespace rather than trying
